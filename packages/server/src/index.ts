  ...
  try {
    ...
    if (!files || files.length === 0) {
      return res.status(HTTP_STATUS.BAD_REQUEST).json(
        createApiResponse(false, null, 'No files uploaded')
      );
    }
    ...
    res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(
      createApiResponse(false, null, ERROR_MESSAGES.UPLOAD_FAILED)
    );
  }
});
...
app.get(API_ENDPOINTS.STATUS + '/:uploadId', (req, res) => {
  ...
  res.status(HTTP_STATUS.OK).json(
    createApiResponse(true, mockStatus)
  );
});
...
app.get(API_ENDPOINTS.RESULTS + '/:uploadId', (req, res) => {
  ...
  res.status(HTTP_STATUS.OK).json(
    createApiResponse(true, mockResults)
  );
});
...
app.use((error: Error, req: express.Request, res: express.Response, next: express.NextFunction) => {
  ...
  if (error.message.includes('File too large')) {
    return res.status(HTTP_STATUS.PAYLOAD_TOO_LARGE).json(
      createApiResponse(false, null, ERROR_MESSAGES.FILE_TOO_LARGE)
    );
  }
  
  res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(
    createApiResponse(false, null, ERROR_MESSAGES.SERVER_ERROR)
  );
});
...
if (process.env.NODE_ENV !== 'test') {
  app.listen(PORT, () => {
    ...
    console.log(`📁 Upload endpoint: http://localhost:${PORT}${API_ENDPOINTS.UPLOAD}`);
  });
}
