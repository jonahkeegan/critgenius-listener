...
export const useResponsiveLayout = () => {
  ...
  const currentBreakpoint = useMemo(():
    ...
    // Use existing booleans to avoid re-invoking useMediaQuery within memo
    if (!isTablet && !isDesktop && !isLargeScreen && !isMobile) return 'sm';
...
export const useFluidSpacing = () => {
  ...
  const getSpacing = (
    category: 'containerPadding' | 'sectionSpacing' | 'componentSpacing'
  ) => {
    const spacingObj = responsiveSpacing[category];
    const key: keyof typeof spacingObj = currentBreakpoint;
  ...
  const spacingUtils = useMemo(
    () => ({
      ...
      audioInterface: {
        controlSpacing:
          responsiveSpacing.audioInterface.controlSpacing[
            currentBreakpoint as keyof typeof responsiveSpacing.audioInterface.controlSpacing
          ] || responsiveSpacing.audioInterface.controlSpacing.md,
        visualizerHeight:
          responsiveSpacing.audioInterface.visualizerHeight[
            currentBreakpoint as keyof typeof responsiveSpacing.audioInterface.visualizerHeight
  ...
  const layoutConfig = useMemo(
    () => ({
      ...
      transcriptDisplay: {
        ...
        fontSize: isMobile
          ? 'clamp(0.875rem, 2.8vw, 1.125rem)'
          : 'clamp(0.875rem, 2.8vw, 1.25rem)',
