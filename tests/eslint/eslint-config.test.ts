...
describe('ESLint accessibility rule regression', () => {
  ...
  const resultInvalid = await runLint('tests/eslint/__fixtures__/a11y-invalid.tsx');
  const ruleIds = (resultInvalid!.messages as Linter.LintMessage[]).map(m => m.ruleId).filter((r): r is string => Boolean(r));
    ...
    expect(ruleIds.some(r => ['jsx-a11y/click-events-have-key-events','jsx-a11y/interactive-supports-focus'].includes(r!))).toBe(true);
  });

  it('does not flag valid accessibility patterns', async () => {
  const resultValid = await runLint('tests/eslint/__fixtures__/a11y-valid.tsx');
  const errorRuleIds = (resultValid!.messages as Linter.LintMessage[]).map(m => m.ruleId).filter((r): r is string => Boolean(r));
